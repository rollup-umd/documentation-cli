  ...
  async.auto({
    ...
    makeDoc: ['runExtract', 'isPrivate', 'setupCi', (res, cb) => {
      ...
      pkg.translation.locales.forEach((locale) => {
        const translationPath = path.join(argv.path, 'translation', `${locale}.json`);
        if (fs.existsSync(translationPath)) {
          ...
          Object.keys(messages).forEach((key) => {
            readme += `| \`${key}\` | ${messages[key] ? `\`${messages[key]}\`` : emptyLink(pkg, locale, isPrivate)} |`;
            readme += '\n';
          });
          readme += '\n';
      ...
      console.log(`${path.join(argv.path, 'docs/translation.md')} has been updated`);
    }]
  }, (err, res) => {
    if (err) {
